...
public class BetteredEnchanting implements ModInitializer {
    ...
    public void onInitialize() {
        ...
        MidnightConfig.init("bettered_enchanting/bettered_enchanting", GlobalConfig.class);
    ...
    private void addEventListner() {
        ...
        ServerLifecycleEvents.SERVER_STARTED.register(e -> {
            ...
            if (e.getResourceManager().getAllNamespaces().contains("toss_up") && !TOSS_UP_PRESENT) {
                LOGGER.info("Datapack Toss Up is present and loaded ; Building compat for " + BetteredEnchanting.MODID);
    ...
    private ItemGroup generateItemGroup() {
        ...
        return FabricItemGroup.builder()
                ...
                .displayName(Text.translatable("itemGroup.bettered_enchanting.item_group"))
                .entries((context, entries) -> {
                    entries.addAll(BEItems.MOD_ITEM_LIST.stream().map(Item::getDefaultStack).toList());

                    if (BetteredEnchanting.BUMBLEZONE_PRESENT)
    ...
    private void checkForCompat() {
        if (FabricLoader.getInstance().isModLoaded("the_bumblezone")) {
            LOGGER.info("Mod Bumblezone is present and loaded ; Building compat for " + BetteredEnchanting.MODID);
            BUMBLEZONE_PRESENT = true;
            ModEnchantIngredientMap.loadBumblezoneConfig();
        }

        if (FabricLoader.getInstance().isModLoaded("incantationem")) {
            LOGGER.info("Mod incantationem is present and loaded ; Building compat for " + BetteredEnchanting.MODID);
        }

        if (FabricLoader.getInstance().isModLoaded("horseshoes")) {
            LOGGER.info("Mod horseshoes is present ; Building compat for " + BetteredEnchanting.MODID);
            HORSESHOES_PRESENT = true;
        }

        if (FabricLoader.getInstance().isModLoaded("mr_dungeons_andtaverns")) {
            LOGGER.info("Mod nova_structures (Dungeon and Taverns) is present and loaded ; Building compat for " + BetteredEnchanting.MODID);
            DUNGEONS_AND_TAVERNS_PRESENT = true;
        }

        if (FabricLoader.getInstance().isModLoaded("toss_up")) {
            LOGGER.info("Mod Toss Up is present and loaded ; Building compat for " + BetteredEnchanting.MODID);
            TOSS_UP_PRESENT = true;
            ModEnchantIngredientMap.loadTossUpConfig();
        }

        if (FabricLoader.getInstance().isModLoaded("diversity")) {
            LOGGER.info("Mod Diversity is present and loaded; Building compat for " + BetteredEnchanting.MODID);
            DIVERSITY_PRESENT = true;
            ModEnchantIngredientMap.loadDiversityConfig();
        }

        if (FabricLoader.getInstance().isModLoaded("spell_power")) {
            LOGGER.info("Mod Spell Power is present and loaded; Building compat for " + BetteredEnchanting.MODID);
            SPELL_POWER_PRESENT = true;
            ModEnchantIngredientMap.loadSpellPowerConfig();
        }

        if (FabricLoader.getInstance().isModLoaded("combat_roll")) {
            LOGGER.info("Mod Combat Roll is present and loaded; Building compat for " + BetteredEnchanting.MODID);
