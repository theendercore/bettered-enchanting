...
import cutefox.bettered_enchanting.data.ModItemTags;
...
public class BETradeOffers extends TradeOffers {
    ...
    private static Factory createLibrarianTradeIngredientFactory(int experience) {
        return new BETypedWrapperFactory(Map.of(
    ...
    private record BETypedWrapperFactory(Map<VillagerType, Factory> typeToFactory) implements Factory {

        public static BETypedWrapperFactory of(Factory factory, VillagerType... types) {
            return new BETypedWrapperFactory((Map) Arrays.stream(types).collect(Collectors.toMap((type) -> {
                return type;
            }, (type) -> {
                return factory;
            })));
