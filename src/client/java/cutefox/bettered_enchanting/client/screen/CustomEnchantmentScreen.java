package cutefox.bettered_enchanting.client.screen;
...
public class CustomEnchantmentScreen extends HandledScreen<CustomEnchantmentScreenHandler> {
    private static final Identifier ENCHANTMENT_BOOK_DISABLED = BetteredEnchanting.id("container/enchanting_table/enchantment_book_disabled");
    private static final Identifier ENCHANTMENT_BOOK_HIGHLIGHTED = BetteredEnchanting.id("container/enchanting_table/enchantment_book_highlighted");
    private static final Identifier SCROLLER = BetteredEnchanting.id("container/enchanting_table/scroller");
    private static final Identifier SCROLLER_DISABLED = BetteredEnchanting.id("container/enchanting_table/scroller_disabled");
    private static final Identifier BOOK_SLOT_SELECTOR = BetteredEnchanting.id("container/enchanting_table/book_slot_selector");
    private static final Identifier BOOK_GRAY_OVERLAY = BetteredEnchanting.id("container/enchanting_table/book_gray_overlay");
    private static final Identifier ENCHANTING_TABLE_BACKGROUND = BetteredEnchanting.id("textures/gui/container/custom_enchanting_table.png");
    private static final Identifier MAGIC_SHARD_FULL = BetteredEnchanting.id("container/enchanting_table/magic_shard_full");
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        if (handler.enchantmentId[0] == -5) {
            if (this.isPointWithinBounds(72, 14, 15, 15, mouseX, mouseY)) {
                ...
                mutableText3 = Text.translatable("container.bettered_enchanting.enchant.charge");
                        ...
                        if (this.isPointWithinBounds(72 + (16 * l) + (4 * l), 14 + (16 * (k - indexStartOffset)), 16, 14, mouseX, mouseY)
                            ...
                            if (!isInCreative && !hasEnchantLevel) {
                                ...
                                if (this.client.player.experienceLevel < enchantLevReq) {
                                    ...
                                    mutableText3 = Text.translatable("container.bettered_enchanting.enchant.material.one", enchantIngredientCost, Text.translatable(enchantIngredientStack.getTranslationKey()));
